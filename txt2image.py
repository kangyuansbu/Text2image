...
def main_train():
    g_real_image = tf.placeholder('float32', [batch_size, image_size, image_size, 3], name = 'real_image')
    g_wrong_image = tf.placeholder('float32', [batch_size ,image_size, image_size, 3], name = 'wrong_image')
    t_real_caption = tf.placeholder(dtype=tf.int64, shape=[batch_size, None], name='real_caption_input')
    t_wrong_caption = tf.placeholder(dtype=tf.int64, shape=[batch_size, None], name='wrong_caption_input')
    t_z = tf.placeholder(tf.float32, [batch_size, z_dim], name='z_noise')

    ## training inference for text-to-image mapping
    net_cnn = cnn_encoder(g_real_image, is_train=True, reuse=False)
    x = net_cnn.outputs
    v = rnn_embed(t_real_caption, is_train=True, reuse=False).outputs
    x_w = cnn_encoder(g_wrong_image, is_train=True, reuse=True).outputs
    ...
    _, disc_real_image_logits = discriminator_txt2img(
                    g_real_image, net_rnn.outputs, is_train=True, reuse=True)
        ...
        for step in range(n_batch_epoch):
            ...
            if epoch < 50:
                errRNN, _ = sess.run([rnn_loss, rnn_optim], feed_dict={
                                                g_real_image : b_real_images,
                                                g_wrong_image : b_wrong_images,
